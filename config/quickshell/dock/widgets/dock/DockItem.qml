...
Rectangle {
  ...
  // NEW: Calculate icon scale based on hover
  property real iconScale: {
    if (row.current == -1) return 1.0
    const falloff = config.data.falloff || 3
    let diff = Math.abs(index - row.current)
    if (diff > falloff) return 1.0
    
    // Scale factor for icon size (adjust this value to control scaling intensity)
    const maxScale = 1.3  // Maximum scale (60% larger)
    const scaleCurve = 1.0 - (diff / falloff)
    return 1.0 + (maxScale - 1.0) * scaleCurve
  }
    ...
    Column {
      ...
      Rectangle {
        ...
        Image {
          ...
          // MODIFIED: Combined press and hover scaling
          transform: Scale {
            origin.x: config.data.iconSize / 2
            origin.y: config.data.iconSize / 2
            xScale: itemMouseArea.containsPress ? 0.9 : iconScale
            yScale: itemMouseArea.containsPress ? 0.9 : iconScale
